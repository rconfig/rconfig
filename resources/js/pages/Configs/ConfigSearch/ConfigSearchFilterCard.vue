  ...
  <div>
    <Card class="m-2 overflow-hidden">
      ...
      <CardContent class="p-4 pt-0 text-sm">
        ...
        <transition name="fade">
          <div class="grid gap-3" v-if="!isLoading">
            <div class="grid items-center">
              <Select v-model="model.command">
                ...
                <SelectContent class="w-full mb-1 focus:outline-none focus-visible:ring-0">
                  <SelectGroup>
                    <SelectItem v-for="command in commands" :value="command.command">
                      {{ command.command }}
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
            ...
            <div class="grid items-center">
              <Input v-model="model.search_string" id="search_string" autocomplete="off" placeholder="Search String" class="w-full mt-4 mb-1 focus:outline-none focus-visible:ring-0" />
              <p class="ml-1 text-xs text-muted-foreground">
                Required
                <span class="text-red-400">*</span>
              </p>
            ...
            <div class="grid items-center">
              <Input v-model="model.device_name" id="device_name" autocomplete="off" placeholder="Device Name" class="w-full mt-4 mb-1 focus:outline-none focus-visible:ring-0" />
              <p class="ml-1 text-xs text-muted-foreground">Enter full or partial device name</p>
            </div>
            <div class="grid items-center">
              <Input v-model="model.device_category" id="device_category" autocomplete="off" placeholder="Command Group" class="w-full mt-4 mb-1 focus:outline-none focus-visible:ring-0" />
            ...
            <div class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-2">
              <div class="flex flex-col items-start">
                <Input type="number" id="lines_before" v-model="model.lines_before" />
                <Label for="lines_before" class="mt-2 text-sm font-medium leading-none cursor-pointer peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Lines Before
                </Label>
              </div>
              <div class="flex flex-col items-start">
                <Input type="number" id="lines_after" v-model="model.lines_after" />
                <Label for="lines_after" class="mt-2 text-sm font-medium leading-none cursor-pointer peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Lines After
                </Label>
              </div>
            </div>
            ...
            <div class="flex flex-col items-start w-full space-x-2">
              <ConfigSearchFilterCardDateRangePicker class="w-full" @dateChange="setDates($event)" />
      ...
      <CardFooter class="flex flex-row items-center justify-end px-4 py-2 border-t bg-muted/50">
        <Button variant="outline" @click="clearAll()" class="flex items-center h-6 px-2 py-1 ml-2 text-xs text-muted-foreground" v-if="!isLoading">
          Clear
        </Button>
        <Button variant="primary" @click="performSearch()" class="h-6 px-2 py-1 ml-2 text-xs bg-blue-600 hover:bg-blue-700 hover:animate-pulse" v-if="!isLoading">
          Search
        </Button>
      </CardFooter>
    </Card>
