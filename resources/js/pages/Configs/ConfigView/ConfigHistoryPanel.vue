	...
	<div>
		<Card class="lg:col-span-6 border-0 shadow-none rounded-2xl bg-transparent text-card-foreground">
					...
					<CardTitle class="gap-2 text-lg group">
						<div class="flex justify-between">
							...
							<div class="flex items-center gap-2">
								<RcIcon
									name="refresh"
									class="w-4 h-4 text-muted-foreground cursor-pointer hover:text-rcgray-200 transition-transform"
			...
			<CardContent class="p-4 pt-0 text-sm">
				...
				<div
					v-if="shouldShowRefreshSkeleton"
					class="space-y-4"
				>
					<div
						v-for="i in 3"
				...
				<!-- Empty State -->
				<div
					v-else-if="!shouldShowRefreshSkeleton && (!configsData || !configsData.data || configsData.data.length === 0)"
				...
				<transition>
					<div
						v-if="!shouldShowRefreshSkeleton && configsData && configsData.data && configsData.data.length > 0"
						key="content"
						class="space-y-2"
					>
						<div
							v-for="(config) in configsData.data"
						...
						>
							<!-- Collapsed Header -->
							<div
								class="flex items-center justify-between p-3 cursor-pointer hover:bg-muted/10 dark:hover:bg-muted/5 transition-colors duration-150"
								@click="toggleExpand(config.id)"
							>
								<div class="flex items-center gap-3">
									<div
										class="w-2 h-2 rounded-full"
										:class="[config.latest_version === 1 ? 'bg-green-500' : 'bg-muted-foreground/40', isConfigBeingViewed(config.id, config.config_version) ? 'bg-blue-500' : '', isConfigShowingChanges(config.id) ? 'bg-orange-500' : '']"
									></div>
									<div>
										<div class="flex items-center gap-2">
											<span
												class="font-medium text-sm"
												:class="[isConfigBeingViewed(config.id, config.config_version) ? 'text-blue-600 dark:text-blue-400' : '', isConfigShowingChanges(config.id) ? 'text-orange-600 dark:text-orange-400' : '']"
											>v{{ config.config_version }}</span>
											<span class="text-xs text-muted-foreground">ID: {{ config.id }}</span>
											<RcBadge
												v-if="config.latest_version === 1"
												variant="success"
												class="text-xs leading-none px-2 py-0.5 h-6 flex items-center"
											>
												CURRENT
											</RcBadge>
											<RcBadge
												v-if="isConfigShowingChanges(config.id)"
												variant="default"
												class="text-xs leading-none px-2 py-0.5 h-6 flex items-center gap-1 bg-orange-500 text-white"
											>
										...
										<div class="text-xs text-muted-foreground">
											{{ formatters.formatTime(config.created_at) }} • {{ formatters.formatFileSize(config.config_filesize) }}
										</div>
									</div>
								</div>
								...
								<div class="flex items-center">
									<GitCommit
										class="w-4 h-4 text-muted-foreground transform transition-transform"
										:class="expandedConfigs.includes(config.id) ? 'rotate-180' : ''"
									/>
								</div>
							</div>
							...
							<!-- Expanded Details -->
							<div
								v-if="expandedConfigs.includes(config.id)"
								class="px-3 pb-3"
							>
								<div class="border rounded-lg p-3 bg-muted/50 dark:bg-[rgb(27,29,33)]">
									<div class="grid grid-cols-1 gap-4 text-sm mb-3">
										...
										<div class="grid grid-cols-2 gap-4">
											<div class="min-w-0">
												...
												<span
													class="block truncate text-xs mt-1"
													:title="config.config_filename"
												>{{ config.config_filename }}</span>
											</div>
											<div>
										...
										<div class="grid grid-cols-2 gap-4">
											<div v-if="config.duration">
												<span class="font-medium">Duration:</span> {{ config.duration }}s
											</div>
											<div v-if="config.connection_type">
												<span class="font-medium">Connection:</span> {{ config.connection_type }}
											</div>
										</div>
									</div>
									...
									<div class="flex gap-2">
										<Button
											...
											variant="outline"
											class="text-xs"
											:class="isConfigBeingViewed(config.id, config.config_version) && !isConfigShowingChanges(config.id) ? 'border-blue-500 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10' : ''"
											:disabled="isConfigBeingViewed(config.id, config.config_version) && !isConfigShowingChanges(config.id)"
											@click="onViewConfig(config.id, config.config_version)"
										>
										...
										<Button
											...
											variant="outline"
											class="text-xs"
											:class="isConfigShowingChanges(config.id) ? 'border-orange-500 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-500/10' : ''"
											:disabled="isConfigShowingChanges(config.id)"
											@click="onViewConfigChanges(config.id)"
										>
											<GitBranch
												v-if="!isConfigShowingChanges(config.id)"
												class="w-3 h-3 mr-1"
			...
			<CardFooter
				v-if="!shouldShowRefreshSkeleton && configsData && configsData.total > 0"
				class="px-2 py-2 border-t bg-muted/50"
			>
				<div class="w-full">
					<Pagination
