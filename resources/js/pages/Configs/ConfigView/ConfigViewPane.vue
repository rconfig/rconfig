			...
			>
				...
				>
					<ScrollArea class="max-h-[83vh] w-full rounded-md border smooth-scroll overflow-y-auto">
						...
						/>
						<ConfigHistoryPanel
							v-if="leftNavSelected === 'configHistory'"
							:is-loading="isLoading"
							:config-data="configData"
							@view-config="handleViewConfig"
							@view-config-changes="handleViewConfigChanges"
						/>
					</ScrollArea>
				</ResizablePanel>
				...
				<ResizablePanel class="min-h-[100vh] border-t">
					<ScrollArea class="h-full border border-none rounded-md">
						...
						<ConfigViewMainPanel
							v-if="!isLoading && configData && !showChangesPanel"
							:key="viewConfigId"
							class="p-2"
							:device-id="configData.device_id"
							:device-name="configData.device_name"
							:config-id="viewConfigId"
							:selected-config-version="selectedConfigVersion || configData.config_version"
							style="height: 60vh;"
						/>
						<ConfigChangesPanel
							v-if="!isLoading && configData && showChangesPanel"
							:key="'changes-' + changesConfigId"
							class="p-2"
							:config-id="changesConfigId"
