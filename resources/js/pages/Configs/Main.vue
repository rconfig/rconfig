...
<template>
	<main class="flex flex-col flex-1 gap-2 dark:bg-rcgray-900">
		...
		<div>
			<keep-alive>
				<component 
					:is="currentView === 'configs' ? ConfigsTable : currentView === 'configsearch' ? ConfigSearch : currentView === 'configcompare' ? ConfigCompare : null" 
					:configsId="configsId" 
					...
					<template v-if="['configsearch', 'configcompare'].includes(currentView)" #default>
						<h1 class="text-2xl text-muted-foreground">
							{{ currentView === 'configsearch' ? 'Config Search' : 'Config Compare' }}
						</h1>
					</template>
				</component>
			</keep-alive>
