...
import { Sparkles, CornerDownLeft, X } from "lucide-vue-next";
...
} = useAddEditDevices(props.editId, emit, props.isClone);
...
function updateDeviceCredFields(cred) {
	...
	model.value.device_cred_id = cred.id;
	model.value.device_username = cred.cred_username;
	model.value.device_password = "";
	model.value.device_enable_password = "";
}
function removeDeviceCredSet() {
	model.value.device_cred = null;
	model.value.device_cred_id = 0;
	model.value.device_username = "";
	model.value.device_password = "";
	model.value.device_enable_password = "";
}
</script>
