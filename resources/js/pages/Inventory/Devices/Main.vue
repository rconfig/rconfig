	...
	<div class="flex flex-col h-full gap-1 text-center">
		<div class="flex items-center justify-between p-4">
			<div class="flex items-center">
				...
				<div class="flex gap-2">
					...
					<ClearFilters
						v-if="searchTerm || filterCategories.length || filterTags.length || filterVendor.length || filterModel.length || filterStatus.length"
			...
			<div class="flex items-center justify-end">
				...
				<ColumnChooser
					...
					:columns="columnDefs"
					storage-key="rconfig.deviceTable.columns"
				/>
				...
				<DropdownMenu>
					...
					>
						<DropdownMenuItem
							class="cursor-pointer hover:bg-rcgray-600 group"
						...
						>
							...
							<DropdownMenuShortcut class="group">
								<CirclePlus
									size="16"
									class="text-rcgray-400 group-hover:text-rcgray-300"
								/>
							</DropdownMenuShortcut>
						</DropdownMenuItem>
						<DropdownMenuItem
							class="cursor-pointer hover:bg-rcgray-600 group"
							@click.prevent="exportInventoryReport()"
						>
							Inventory Report
							<DropdownMenuShortcut class="group">
								<FileStack
									v-if="!inventoryIsLoading"
									size="16"
									class="text-rcgray-400 group-hover:text-rcgray-300"
								/>
								<Spinner
									:state="inventoryIsLoading"
									class="mr-2"
								/>
							</DropdownMenuShortcut>
						</DropdownMenuItem>
				...
				<Button
					type="submit"
					class="px-2 py-1 ml-2 text-sm bg-blue-600 hover:bg-blue-700 hover:animate-pulse hidden xl:inline-flex"
					size="sm"
					variant="primary"
					@click.prevent="createDevice()"
				>
				...
				<Button
					type="submit"
					class="px-2 py-1 ml-2 text-sm hidden xl:inline-flex"
					size="sm"
					variant="outline"
					@click.prevent="exportInventoryReport()"
				>
		...
		<div class="px-6">
			<Table>
				<TableHeader>
					<TableRow>
						...
						<!-- Device Name column -->
						<TableHead
							v-if="isColumnVisible('device_name')"
							class="w-[20%]"
						>
							<Button
								class="flex justify-start w-full p-0 hover:bg-rcgray-800"
						...
						<!-- IP Address column -->
						<TableHead
							v-if="isColumnVisible('device_ip')"
							class="w-[10%]"
						>
							IP Address
						</TableHead>
						...
						<!-- Model column -->
						<TableHead
							v-if="isColumnVisible('device_model')"
							class="w-[10%]"
						>
							Model
						</TableHead>
						...
						<!-- Config Count column -->
						<TableHead
							v-if="isColumnVisible('config_good_count')"
							class="w-[10%]"
						>
							Config Count
						</TableHead>
						...
						<TableHead
							v-if="isColumnVisible('config_bad_count')"
							class="w-[10%]"
						>
							Config Failures
						</TableHead>
						...
						<!-- Last Config column -->
						<TableHead
							v-if="isColumnVisible('last_config')"
							class="w-[10%]"
						>
							Last Config
						</TableHead>
				...
				<TableBody>
					...
					<template v-else-if="!isLoading">
						...
						>
							...
							<TableCell
								v-if="isColumnVisible('checkbox')"
								class="text-start table-cell-fixed"
							>
								<Checkbox
									:id="'select-' + row.id"
							...
							>
								<div class="flex items-center">
									...
									<RcIcon
										v-if="row.device_added_by && row.device_added_by >= 9000"
										name="imported-device"
										:added-by="row.device_added_by"
										:source="row.device_added_by === 9001 ? 'Netbox' : row.device_added_by === 9000 ? 'Zabbix' : 'Integration'"
									/>
								</div>
							</TableCell>
							...
							>
								...
								<RcIcon
									v-if="row.status === 300"
									name="status-red-agent"
									size="16"
								/>
								<RcIcon
									v-if="row.status === 301"
									name="status-gray-agent"
								/>
							</TableCell>
							...
							>
								<span
									...
									class="text-truncate"
									:title="row.category[0].categoryName"
								>{{ row.category[0].categoryName }}</span>
								<span v-else>--</span>
							</TableCell>
							...
							>
								<span
									v-if="row.vendor.length > 0"
									class="text-truncate"
									:title="row.vendor[0].vendorName"
								>{{ row.vendor[0].vendorName }}</span>
								<span v-else>--</span>
							</TableCell>
							...
							>
								<span
									v-if="row.last_config"
									class="text-truncate"
									:title="formatters.formatTime(row.last_config.created_at)"
								>{{ formatters.formatTime(row.last_config.created_at) }}</span>
								<span v-else>--</span>
							</TableCell>
							...
							>
								<div class="badge-container-fixed">
									<BadgeList
										...
										link-field="view_url"
										:max-visible="1"
										:hover-card-fields="['id', 'tagname', 'description']"
									/>
								</div>
							</TableCell>
							...
							>
								<div class="button-cell-fixed">
									<ActionsMenuDevices
										:row-data="row"
										:show-copy-download-debug="true"
										@on-enable="enableDevice(row.id)"
										@on-clone="viewEditDialog(row.id), (isClone = true)"
										@on-disable="disableDevice(row.id)"
										@on-edit="viewEditDialog(row.id), (isClone = false)"
										@on-delete="deleteDevice(row.id)"
									/>
								</div>
							</TableCell>
						</TableRow>
			...
			>
				<span class="text-sm">
					<RcBadge
						variant="primary"
						class="mr-1"
					>{{ selectedRows.length }} </RcBadge> Devices Selected
				</span>
				<Popover v-slot="{ open }">
					<PopoverTrigger class="col-span-3">
						<Button
							class="text-sm px-1.5 py-0 h-8 flex items-center gap-1 group"
							variant="outline"
						>
							<Power
								class="inline-block mr-1 ml-0"
								size="14"
							/>
							<span class="hidden md:inline-flex">Enable/Disable</span>
							<ChevronDown
								class="transition-transform duration-200 ease-in-out group-hover:rotate-180"
					...
					<PopoverContent
						side="bottom"
						align="start"
						class="col-span-3 p-1 text-left w-max"
					>
						<div class="flex flex-col items-start gap-1">
							<Button
								class="text-sm px-1.5 py-0 h-8 hover:animate-pulse w-full"
								variant="ghost"
								@click.prevent="enableManyDevices(selectedRows)"
							>
								<TvMinimalPlay
									size="16"
									class="text-green-300 mr-1"
								/>
								Enable Selected
							</Button>
							<Button
								class="text-sm px-1.5 py-0 h-8 w-full"
								variant="ghost"
								@click.prevent="disableManyDevices(selectedRows)"
							>
								<MonitorOff
									size="16"
				...
				<Button
					...
					variant="outline"
					@click.prevent="downloadManyDevices(selectedRows)"
				>
					<RcIcon
				...
				<Button
					class="text-sm px-1.5 py-0 hover:animate-pulse h-8"
					variant="outline"
					@click.prevent="openBulkEditDialog()"
				>
					<FilePenLine
						class="inline-block mr-0 md:mr-2 ml-0"
						size="14"
			...
			<DeviceBulkEditDialog
				...
				:key="newBulkEditModalKey"
				:checked-rows="selectedRows"
				@bulk-update-success="reload()"
			/>
			...
			<RcConfirmAlertDialog
				...
				@handle-delete="deleteManyDevices(selectedRows)"
			/>
