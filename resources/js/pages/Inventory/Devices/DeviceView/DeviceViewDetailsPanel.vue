...
<template>
	<div>
		<Card class="overflow-hidden">
			<CardHeader
				class="py-2 px-4 bg-muted/50 cursor-pointer"
			...
			>
				<div class="flex flex-row items-center justify-between">
					...
					>
						...
						<div
							v-if="deviceData.status === 0"
							class="flex items-center px-2 py-1 rounded-md bg-red-500/10 border border-red-500/20"
						>
							<RcIcon
								name="status-red"
								class="mr-2"
						...
						<!-- Green status - Online -->
						<div
							v-else-if="deviceData.status === 1"
							class="flex items-center px-2 py-1 rounded-md bg-green-500/10 border border-green-500/20"
						>
							<RcIcon
								name="status-green"
								class="mr-2"
						...
						<div
							v-else-if="deviceData.status === 2"
							class="flex items-center px-2 py-1 rounded-md bg-yellow-500/10 border border-yellow-500/20"
						>
							<RcIcon
								name="status-yellow"
								class="mr-2"
						...
						<!-- Gray status - Unknown -->
						<div
							v-else-if="deviceData.status === 100"
							class="flex items-center px-2 py-1 rounded-md bg-gray-500/10 border border-gray-500/20"
						>
							<RcIcon
								name="status-gray"
								class="mr-2"
					...
					>
						<dl class="grid gap-3">
							...
							<div class="flex items-center justify-between">
								...
								<dd class="flex items-center gap-2">
									<router-link
										:to="`/commandgroups/${deviceData.category[0]?.id}`"
										class="rc-text-link"
									>
										<span
											:class="deviceData.category[0]?.badgeColor ? deviceData.category[0]?.badgeColor : 'bg-gray-600 text-gray-200 border-gray-500'"
											class="w-fit flex items-center text-xs font-medium px-2.5 py-0.5 rounded-xl border"
										>
							...
							<div class="flex items-center justify-between">
								...
								<dd class="flex items-center gap-2">
									<router-link
										:to="`/vendors/${deviceData.vendor[0]?.id}`"
										class="rc-text-link"
									>
							...
							<div class="flex items-center justify-between">
								...
								<dd class="flex items-center gap-2">
									<router-link
										:to="{ name: 'template-view', params: { id: deviceData.template[0]?.id } }"
										class="rc-text-link"
									>
										{{ deviceData.template[0]?.templateName }}
									</router-link>
								</dd>
							</div>
							<div class="flex max-w-full items-start gap-2">
								<dt class="w-[6rem] text-muted-foreground shrink-0">
									Tags
								</dt>
								<dd class="flex flex-wrap justify-end gap-2 w-full">
									<div
										v-for="tag in deviceData.tag"
										:key="tag.id"
							...
							<div class="flex items-center justify-between">
								...
								<dd class="flex items-center gap-2">
									{{ formatters.formatTime(deviceData.created_at) }}
								</dd>
							</div>
						</dl>
					</div>
