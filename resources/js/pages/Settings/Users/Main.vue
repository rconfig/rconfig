	...
	<div class="flex flex-col h-full gap-1 text-center">
		...
		<div class="px-6">
				...
				<TableBody>
					...
					<template v-else-if="!isLoading">
						...
						>
							...
							<TableCell class="text-start">
								<RcBadge
									v-if="row.is_socialite"
									variant="secondary"
									class="ml-2 px-2 flex items-center gap-1"
									title="SSO User"
								>
							...
							<TableCell class="text-start">
								...
								<RcToolTip
									v-else
									:delay-duration="100"
									:content="'No role assigned to this user'"
									side="bottom"
								>
									<template #trigger>
										<RcBadge
							...
							<TableCell class="text-start">
								<Switch
									:id="`notif-${row.id}`"
									v-model:checked="row.get_notifications"
									@update:checked="toggleNotification(row.id, $event ? 1 : 0)"
								/>
							</TableCell>
							<TableCell class="text-start">
								<Switch
									:id="`socialite-${row.id}`"
									v-model:checked="row.is_socialite_approved"
									@update:checked="toggleSocialiteApproved(row.id, $event ? 1 : 0)"
								/>
							</TableCell>
							<TableCell class="text-start">
								{{ row.last_login }}
			...
			<RcConfirmAlertDialog
				...
				@handle-delete="deleteManyUsers(selectedRows)"
			/>
			<RelatedDocumentationNav
