...
<template>
	<Card class="bg-transparent border-none">
		<CardHeader>
			...
			<AlertWarning
				v-if="notificationStatus === false"
				:title="'Your notifications are disabled'"
				:message="'Please enable notifications in the users settings to receive updates.'"
				:small="true"
				class="my-2"
			/>
		</CardHeader>
			...
			>
				...
				<div class="bg-muted/50 rounded-lg p-4">
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
						<div
							v-for="channel in notificationStore.channels"
							:key="channel.key"
						...
						>
							<component
								:is="channel.icon"
								class="h-4 w-4"
								:class="`text-${channel.color}-600`"
							/>
							<div>
								<span class="text-sm font-medium flex items-center">
									<RcIcon
										v-if="channel.label === 'Database'"
										name="Database"
										class="mr-2"
									/>
									<RcIcon
										v-if="channel.label === 'Email'"
										name="Email"
										class="mr-2"
									/>
									{{ channel.label }}
				...
				<div class="rounded-lg border border-border bg-card overflow-hidden">
					...
					<div class="divide-y divide-border">
						<template
							v-for="category in notificationStore.notificationCategories"
							:key="category.key"
						>
							<div
							...
							>
								...
								>
									...
									<div class="flex justify-center min-w-[80px]">
										<div class="flex items-center">
											<Loader2
												v-if="notificationStore.isUpdating(type.key, 'db')"
												class="h-4 w-4 animate-spin text-muted-foreground"
											/>
											<Checkbox
												v-else
												:checked="notificationStore.getPreference(type.key, 'db')"
												@update:checked="notificationStore.updatePreference(type.key, 'db', $event)"
											/>
										</div>
									</div>
									...
									<div class="flex justify-center min-w-[80px]">
										<div class="flex items-center">
											<Loader2
												v-if="notificationStore.isUpdating(type.key, 'mail')"
												class="h-4 w-4 animate-spin text-muted-foreground"
											/>
											<Checkbox
												v-else
												:checked="notificationStore.getPreference(type.key, 'mail')"
												:disabled="!type.channels.includes('mail')"
												@update:checked="notificationStore.updatePreference(type.key, 'mail', $event)"
											/>
										</div>
									</div>
								</div>
