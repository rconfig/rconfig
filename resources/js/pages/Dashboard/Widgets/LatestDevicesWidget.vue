...
<template>
	<div class="border-0 shadow-md rounded-2xl bg-card text-card-foreground p-4 transition-all duration-200 hover:shadow-lg relative">
		...
		<div
			v-if="editMode"
			class="absolute top-2 right-2 cursor-move opacity-50 z-10"
		>
		...
		<!-- Loading State -->
		<div
			v-if="isLoadingLatestDevices"
			class="space-y-1 text-sm min-h-[175px]"
		>
			<div
				v-for="i in 5"
				:key="i"
		...
		<div
			v-else
			class="space-y-0.5 text-sm min-h-[175px]"
		>
			<div
			...
			>
				<div class="flex items-center gap-2 min-w-0 flex-1">
					...
					<div class="min-w-0 flex-1">
						<p class="font-medium truncate text-sm leading-tight">
							{{ device.device_name }}
						</p>
			...
			<div
				...
				class="flex items-center justify-center min-h-[175px]"
			>
