	...
	<div class="xl:col-span-6">
		<div class="border-0 shadow-md rounded-2xl bg-card text-card-foreground p-4 transition-all duration-200 hover:shadow-lg h-fit">
			<div class="flex items-center justify-between mb-3">
				...
				>
					<RcIcon
						name="refresh"
						size="16"
						:class="isLoadingHealth ? 'animate-spin' : ''"
					/>
				</button>
			</div>
			...
			<div
				...
				class="space-y-3 text-sm min-h-[175px]"
			>
				<div
					v-for="i in 5"
					:key="i"
			...
			<div
				v-else-if="healthLatest.data"
				class="space-y-3 text-sm min-h-[175px]"
			>
				<!-- CPU Load -->
				<div
					v-for="item in healthLatest.data.filter((h) => h.name === 'CpuLoad')"
					:key="item.name"
					class="flex justify-between items-center py-1"
				>
					<div class="flex items-center gap-2">
				...
				<!-- Horizon Queue -->
				<div
					v-for="item in healthLatest.data.filter((h) => h.name === 'Horizon')"
					:key="item.name"
				...
				>
					...
					<div class="flex items-center gap-2">
						...
						<div
							...
							:class="item.status === 'Running' ? 'bg-green-500' : 'bg-red-500'"
						></div>
					</div>
				</div>
				...
				<!-- Disk Space -->
				<div
					v-for="item in healthLatest.data.filter((h) => h.name.includes('Disk Space'))"
					:key="item.name"
					class="flex justify-between items-center py-1.5"
				>
					<div class="flex items-center gap-2">
					...
					<div class="flex items-center gap-2">
						...
						<div
							class="w-2 h-2 rounded-full"
							:class="parseInt(item.status) < 80 ? 'bg-green-500' : 'bg-red-500'"
						></div>
					</div>
				</div>
				...
				<!-- Ping -->
				<div
					v-for="item in healthLatest.data.filter((h) => h.name === 'Ping')"
					:key="item.name"
					class="flex justify-between items-center py-1.5"
				>
					<div class="flex items-center gap-2">
					...
					<div class="flex items-center gap-2">
						...
						<div
							class="w-2 h-2 rounded-full"
							:class="item.status === 'Reachable' ? 'bg-green-500' : 'bg-red-500'"
						></div>
					</div>
				</div>
				...
				<!-- Database -->
				<div
					v-for="item in healthLatest.data.filter((h) => h.name === 'Database')"
					:key="item.name"
					class="flex justify-between items-center py-1.5"
				>
					<div class="flex items-center gap-2">
			...
			<div
				v-else
				class="space-y-3 min-h-[175px] flex items-center justify-center"
			>
			...
			<div class="mt-4 pt-3 border-t border-muted/50 text-xs text-muted-foreground flex items-center justify-between">
				<div class="flex items-center gap-2">
					...
					<div
						v-else
						class="h-3 bg-muted rounded w-24 animate-pulse"
					></div>
				</div>
				...
				<HoverCard>
					...
					<HoverCardContent
						class="w-96 bg-gradient-to-r from-card/80 via-card/60 to-card/80 backdrop-blur-md shadow-sm"
						side="top"
						align="end"
					>
						<div class="space-y-3">
							...
							<div
								v-if="healthLatest.data && !isLoadingHealth"
								class="grid grid-cols-1 gap-2 text-xs"
							>
								<div
									v-for="item in healthLatest.data"
									:key="item.name"
									class="flex justify-between items-center py-1"
								>
									...
									<div class="flex items-center gap-2">
										...
										<div
											class="w-2 h-2 rounded-full"
											:class="['Ok', 'Running', 'Reachable'].includes(item.status) || (item.name.includes('Disk') && parseInt(item.status) < 80) ? 'bg-green-500' : 'bg-red-500'"
										></div>
									</div>
								</div>
							</div>
