	...
	<div class="flex flex-col h-full gap-1 text-center">
		...
		<div class="px-6">
				...
				<TableBody>
					...
					<template v-else-if="!isLoading">
						...
						>
							...
							<TableCell
								class="text-start"
								:title="row.is_paused ? `Task ${row.id} is paused` : `Task ${row.id} is active`"
							>
								<div class="flex items-center gap-2">
									<RcIcon
										v-if="row.is_paused"
							...
							<TableCell class="text-start">
								<div class="flex flex-col">
									<Button
										class="px-2 py-0 text-sm hover:bg-rcgray-800 rounded-xl justify-start"
										variant="ghost"
							...
							<TableCell class="text-start">
								<div class="flex items-center gap-1">
									<RcIcon
										name="commands"
										:width="16"
										class="w-4 h-4 text-muted-foreground flex-shrink-0 mr-1"
									/>
							...
							<TableCell class="text-start">
								<div class="flex flex-col gap-1">
									<div
										v-if="row.device && row.device.length > 0"
									...
									<div
										v-if="row.category && row.category.length > 0"
										class="flex items-center gap-1"
									>
										<RcIcon
											name="command-group"
									...
									<div
										v-if="row.task_command && row.task_command.includes('download')"
										class="flex items-center gap-1"
									>
										<RcIcon
											name="download2"
									...
									<span
										v-if="(!row.device || row.device.length === 0) && !row.category && !row.tag"
										class="text-xs text-muted-foreground"
									>No targets</span>
								</div>
							</TableCell>
							...
							<TableCell class="text-start">
								<div class="flex flex-col gap-1">
									...
									<div
										v-if="row.download_report_notify"
										class="flex items-center gap-1"
									>
										<RcIcon
											name="docs"
									...
									<div
										v-if="row.verbose_download_report_notify"
										class="flex items-center gap-1"
									>
										<RcIcon
											name="docs"
									...
									<span
										v-if="!row.task_email_notify && !row.download_report_notify && !row.verbose_download_report_notify"
										class="text-xs text-muted-foreground"
									>None</span>
								</div>
							</TableCell>
							...
							<TableCell class="text-start">
								<ActionsMenu
									:row-data="row"
									:show-task-pause-btn="true"
									:show-view-history-btn="true"
									:task-paused="Boolean(row.is_paused)"
									:show-task-run-now-btn="true"
									@on-run-manual-task="runTaskConfirm(row.id)"
									@on-task-pause="pauseTask(row.id)"
									@on-view-history="viewHistoryDialog(row.id)"
									@on-edit="viewEditDialog(row.id)"
									@on-delete="deleteTask(row.id)"
								/>
							</TableCell>
							<!-- ACTIONS MENU -->
						</TableRow>
			...
			<TaskAddEditDialog
				...
				@save="handleSave()"
			/>
			<DialogViewHistory :task-id="historyTaskId" />
			<RcConfirmAlertDialog
				:show-confirm-confirm-proceed-alert-alert="showConfirmConfirmProceedAlertAlert"
				:edit-id="proceedEditId"
				message="Are you sure you want to run this task manually?"
				@handle-close="showConfirmConfirmProceedAlertAlert = false"
				@handle-confirm="runTaskNow(proceedEditId)"
			/>
			<RcConfirmAlertDialog
				:ids="selectedRows"
				:show-confirm-delete="showConfirmDelete"
				@close="showConfirmDelete = false"
				@handle-delete="deleteManyTasks(selectedRows)"
			/>
		</div>
	</div>
</template>