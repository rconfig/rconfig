	...
	<div class="space-y-3 px-3">
		...
		<ScrollArea class="h-[400px] w-full rounded-md border p-3">
			<div class="space-y-3">
				...
				<div
					v-if="getCategoryTasks('config').length > 0"
					class="result-section"
				>
					<div
						class="flex items-center mb-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-rcgray-600 rounded-md p-1.5"
						@click="toggleCategory('config')"
					>
						<ChevronRight
							v-if="collapsedCategories.config"
							class="w-4 h-4 mr-2 text-blue-300"
						/>
						<ChevronDown
							v-else
							class="w-4 h-4 mr-2 text-blue-300"
					...
					<div
						v-show="!collapsedCategories.config"
