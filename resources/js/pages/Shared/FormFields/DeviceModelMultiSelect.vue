...
const {
  selectedItems: selectedDeviceModels,
  open,
  searchTerm,
  ...
  <Popover>
    <PopoverTrigger class="col-span-3">
      <Button
        ...
        :class="selectedDeviceModels.length === 0 ? ' text-rcgray-400' : ''"
      ...
      >
        ...
        <template v-else class="text-rcgray-400">
          ...
          <span
            v-else-if="props.singleSelect && selectedDeviceModels.length > 0"
          ...
          >
            ...
            <X
              ...
              @click.stop="deleteItemById(selectedDeviceModels[0].id)"
            />
          </span>
          ...
          <template v-else>
            <span
              v-for="model in selectedDeviceModels"
              :key="model.id"
              class="relative my-1 group"
              v-if="selectedDeviceModels.length > 0"
            >
              <span class="flex items-center text-xs font-medium me-2 px-2.5 py-0.5 rounded-xl border">
                ...
                <X
                  ...
                  @click.stop="deleteItemById(model.id)"
                />
              </span>
            </span>
          </template>
      ...
      <ScrollArea class="h-48 max-h-48">
        <div class="py-1">
          ...
          <div
            ...
            class="flex flex-col items-center justify-center text-rcgray-400 text-sm py-4"
          >
            <span class="font-semibold">No Results</span>
            <span class="mt-1">No device models found. You can add a new model below.</span>
          </div>
        </div>
      </ScrollArea>
