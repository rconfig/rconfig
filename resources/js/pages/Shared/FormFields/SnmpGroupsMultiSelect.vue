...
import SnmpGroupsMultiSelectI18N from "@/i18n/pages/Shared/FormFields/SnmpGroupsMultiSelect.i18n.js";
...
import { useSnmpGroups } from "@/pages/Inventory/SnmpProps/useSnmpGroups";

const emit = defineEmits(["update:modelValue"]);
const groups = ref([]);
const { newGroupModalKey } = useSnmpGroups();
const isLoading = ref(true);

const { t } = useComponentTranslations(SnmpGroupsMultiSelectI18N);
...
const props = defineProps({
	...
	placeholder: {
		type: String,
		default: "Select SNMP groups",
	},
});

const { selectedItems: selectedSnmpGroups, open, searchTerm, filteredItems: filteredSnmpGroups, selectItem, deleteItem: deleteItemById } = useMultiSelect({
	items: groups,
...
function fetchGroups() {
	...
	axios.get("/api/snmp-polling-group/?perPage=10000&sort=name").then((response) => {
	...
	<Popover>
		...
		<PopoverContent side="bottom" align="start" class="col-span-3 p-0 max-h-56 overflow-hidden">
			...
			<div class="p-1 border-5">
				<Button variant="ghost" @click="createSnmpGroup()" class="justify-start w-full p-1">
