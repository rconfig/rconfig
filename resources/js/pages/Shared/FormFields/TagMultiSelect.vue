...
<template>
	<Popover>
		<PopoverTrigger class="col-span-3">
			<Button
				variant="ghost"
				class="flex items-start justify-start w-full px-2 py-1 border rounded-xl gap-1 bg-background"
				:class="[selectedTags.length === 0 ? 'text-muted-foreground h-fit whitespace-nowrap' : 'h-auto min-h-fit']"
			...
			>
				...
				>
					...
					<span
						v-else-if="props.singleSelect && selectedTags.length > 0"
					...
					>
						...
						<X
							...
							@click.stop="deleteItem(selectedTags[0].id)"
						/>
					</span>

					<!-- Display multiple selected items -->
					<div
						v-if="!props.singleSelect && selectedTags.length > 0"
						class="flex flex-wrap gap-1 flex-1 overflow-hidden"
					>
						<!-- Show first 3 tags or all tags based on expanded state -->
						<span
							v-for="tag in isExpanded ? selectedTags : selectedTags.slice(0, 3)"
							:key="tag.id"
							class="relative group flex-shrink-0"
						>
							<span
								class="flex items-center text-xs font-medium px-2.5 py-0.5 rounded-xl border whitespace-nowrap max-w-[150px]"
								:class="tag.badgeColor ? tag.badgeColor : 'bg-secondary text-secondary-foreground border-border'"
							>
								<span class="truncate">{{ tag.tagname }}</span>
								<X
									:size="16"
									class="ml-1 cursor-pointer hover:text-primary flex-shrink-0"
						...
						<button
							v-if="selectedTags.length > 3 && !isExpanded"
			...
			<ScrollArea class="h-64">
				<div class="py-1">
					...
					<div
						...
						v-else
						:key="tag.id"
						class="w-full p-1 pl-2 my-1 text-sm rounded-lg hover:bg-accent hover:text-accent-foreground"
