...
<template>
	...
	<Popover>
		...
		<PopoverTrigger class="col-span-3">
			<Button
				variant="ghost"
				class="flex items-start justify-start w-full px-2 py-1 border rounded-xl gap-1"
				:class="[selectedCats.length === 0 ? 'text-rcgray-400 h-fit whitespace-nowrap' : 'h-auto min-h-fit bg-rcgray-700']"
			...
			>
				...
				>
					...
					<span
						v-else-if="props.singleSelect && selectedCats.length > 0"
						:class="selectedCats[0].badgeColor ? selectedCats[0].badgeColor : 'bg-gray-600 text-gray-200 border-gray-500'"
					...
					>
						...
						<X
							...
							@click.stop="deleteItemById(selectedCats[0].id)"
						/>
					</span>

					<!-- Display multiple selected items -->
					<div
						v-if="!props.singleSelect && selectedCats.length > 0"
						class="flex flex-wrap gap-1 flex-1 overflow-hidden"
					>
						<!-- Show first 3 categories or all categories based on expanded state -->
						<span
							v-for="cat in isExpanded ? selectedCats : selectedCats.slice(0, 3)"
							:key="cat.id"
							class="relative group flex-shrink-0"
						>
							<span
								:class="cat.badgeColor ? cat.badgeColor : 'bg-gray-600 text-gray-200 border-gray-500'"
								class="flex items-center text-xs font-medium px-2.5 py-0.5 rounded-xl border whitespace-nowrap max-w-[150px]"
							>
								<span class="truncate">{{ cat.categoryName }}</span>
								<X
									:size="16"
									class="ml-1 cursor-pointer hover:text-white flex-shrink-0"
									@click.stop="deleteItemById(cat.id)"
								/>
							</span>
						</span>
						<!-- Show expandable button if there are more than 3 selected and not expanded -->
						<button
							v-if="selectedCats.length > 3 && !isExpanded"
							class="flex items-center text-xs font-medium px-2.5 py-0.5 rounded-xl border bg-muted text-muted-foreground flex-shrink-0 hover:bg-muted/80 transition-colors cursor-pointer rc-btn-shadow"
							@click.stop="toggleExpanded()"
						>
							+{{ selectedCats.length - 3 }} more
						</button>
		...
		>
			...
			<ScrollArea class="h-64">
				<div class="py-1">
					...
					<template v-else>
						...
						<div
							v-for="cat in groupedCategories.withCommands"
							:key="cat.id"
						...
						<template v-if="groupedCategories.withoutCommands.length > 0">
							...
							<div
								v-for="cat in groupedCategories.withoutCommands"
								:key="cat.id"
								class="w-full p-1 pl-2 my-1 text-sm rounded-lg hover:bg-rcgray-600"
								@click="selectItem(cat)"
							>
									...
									<TooltipProvider>
										<Tooltip>
											<TooltipTrigger as-child>
												<CloudAlert
													:size="14"
													class="mr-4 text-yellow-300 cursor-default"
												/>
											</TooltipTrigger>
			...
			<div class="p-1 border-5">
				<Button
					variant="ghost"
					class="flex items-center justify-start w-full p-1"
