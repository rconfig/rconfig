...
<template>
	<Popover v-model:open="open">
		<PopoverTrigger class="col-span-3">
			<Button
				variant="ghost"
				class="flex items-center justify-start w-full px-2 mr-2 py-1 border rounded-xl whitespace-nowrap h-fit bg-rcgray-700"
				:class="selectedAgents.length === 0 ? ' text-rcgray-400' : ''"
				:style="selectedAgents.length === 0 ? 'padding: 0.45rem' : 'padding: 0.2rem'"
			>
				...
				>
					...
					<span
						v-else-if="props.singleSelect && selectedAgents.length > 0"
					...
					>
						...
						<X
							...
							@click.stop="deleteItem(selectedAgents[0].id)"
						/>
					</span>
						...
						>
							<span class="flex items-center text-xs font-medium me-2 px-2.5 py-0.5 rounded-xl border bg-muted">
								...
								<X
									size="16"
									class="ml-1 cursor-pointer hover:text-primary"
									@click.stop="deleteItem(agent.id)"
								/>
							</span>
						</span>
					</template>
		...
		>
			...
			<ScrollArea class="h-64">
				...
				<div
					v-else-if="filteredAgents.length === 0 && searchTerm"
				...
				>
					<div
						...
						:key="agent.id"
						class="w-full p-2 my-1 text-sm rounded-lg hover:bg-rcgray-600 cursor-pointer"
