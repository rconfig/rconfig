	...
	<Popover>
		...
		<PopoverTrigger class="col-span-3">
			<Button
				...
				:class="selectedCreds.length === 0 ? ' text-rcgray-400' : ''"
			...
			>
				...
				>
					...
					<span
						v-else-if="props.singleSelect && selectedCreds.length > 0"
					...
					>
						...
						<X
							...
							@click.stop="deleteItemById(selectedCreds[0].id)"
						/>
					</span>
						...
						>
							<span class="flex items-center text-xs font-medium me-2 px-2.5 py-0.5 rounded-xl border">
								...
								<X
									...
									@click.stop="deleteItemById(cred.id)"
								/>
							</span>
						</span>
					</template>
