...
<template>
	<Popover>
		<PopoverTrigger class="w-full">
			<Button
				variant="ghost"
				class="flex flex-wrap items-center justify-start w-full p-1 pl-2 border h-fit gap-1"
				:class="selectedCommands.length === 0 ? 'text-muted-foreground' : ''"
			>
				<RcIcon
					name="commands"
				...
				/>
				<span
					v-if="selectedCommands && selectedCommands.length === 0"
					...
					>
						<span class="flex items-center text-xs font-medium px-2.5 py-0.5 rounded-xl border whitespace-nowrap">
							...
							<X
								...
								@click.stop="deleteItem(cmd.command)"
							/>
						</span>
					</span>
				</div>
