	...
	<Popover v-model:open="open">
		<PopoverTrigger class="col-span-3">
			<Button
				...
				:class="selectedRoles.length === 0 ? ' text-rcgray-400' : ''"
				:style="selectedRoles.length === 0 ? 'padding: 0.45rem' : 'padding: 0.2rem'"
			>
				...
				>
					...
					<span
						v-else-if="props.singleSelect && selectedRoles.length > 0"
					...
					>
						...
						<X
							...
							@click.stop="deleteItem(selectedRoles[0].id)"
						/>
					</span>
		...
		>
			...
			<ScrollArea class="h-64">
				...
				<div
					v-else-if="filteredRoles.length === 0 && searchTerm"
