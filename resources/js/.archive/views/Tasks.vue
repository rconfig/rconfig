<template>
  <main
    ...
    <page-header
      ...
      :desc="viewstate.pagedesc">
      <template
        v-slot:header-right-side
        v-if="showRecentFailedJobs">
        <div
          ...
          <div
            ...
            <span class="pf-c-label pf-m-red">
              <span class="pf-c-label__content">{{ recentFailedJobs }} recent failed jobs</span>
              &nbsp;-&nbsp;
              <a
      ...
      <div
        ...
        <div class="pf-c-drawer__main">
          ...
          <data-table
            ...
            @actionLink="viewtaskHistory($event)"></data-table>
          ...
          <side-drawer
            ...
            <template v-slot:form>
              <tasks-form
                ...
                :key="viewstate.editid"></tasks-form>
            </template>
          </side-drawer>
        </div>
      </div>
  ...
  <task-wizard-modal v-if="viewstate.openDrawerState">
    <template v-slot:wizard>
      <tasks-wizard
        ...
        @formsubmitted="formSubmitted($event)"
        :key="viewstate.editid"></tasks-wizard>
    </template>
  </task-wizard-modal>

  <confirmation-modal
    v-if="viewstate.showConfirmModal"
    @closeModal="viewstate.showConfirmModal = false"
    @action1="runTaskNow">
  ...
  <modal-tasks-run-history
    v-if="viewstate.showTaskRunHistoryModal"
    @close="viewstate.showTaskRunHistoryModal = false"
    :task_id="historyTaskId"></modal-tasks-run-history>
</template>
...
export default {
  ...
  setup() {
    ...
    const { models, isLoading, dataTablePageChanged, closeDrawerState, deleteRow, formSubmitted, confirmDelete, destroyModel } = useViewFunctions(viewstate, viewstate.modelName, viewstate.modelObject);
