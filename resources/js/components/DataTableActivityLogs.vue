            ...
            <div class="pf-c-card">
                <data-table-toolbar :pagename="pagename" @searchInput="addFilters($event)" @openDrawer="openDrawer($event)" :newBtnEnabled="newBtnEnabled" :searchInputDisabled="searchInputDisabled">
                    <template v-slot:customFilter>
                        <slot name="customFilter"><activity-log-filter @filter="addFilters($event)"></activity-log-filter></slot>
                    </template>
                ...
                <table class="pf-c-table pf-m-compact pf-m-grid-lg" role="grid" id="resizeMe">
                    ...
                    >
                        <tr role="row">
                            ...
                            <td v-for="header in tabledata.headers" :key="header.label" role="cell" :data-label="header.label">
                                <span class="pf-c-button__icon pf-m-start"> <i v-if="header.key === 'log_name'" :class="activityLogIconTable[data.log_name]"> </i></span>
                                {{ data[header.key] }}
                            </td>
                        ...
                        <tr class="pf-c-table__expandable-row" role="row" :class="{ 'pf-m-expanded': expanded.includes(data.id) }">
                            <td></td>
                            <td></td>
                                ...
                                <div class="pf-c-table__expandable-row-content">
                                    <div class="pf-c-code-block">
                                        <div class="pf-c-code-block__header">
                                            <div class="pf-c-code-block__actions">
                                                <div class="pf-c-code-block__actions-item">
                                                    <button class="pf-c-button pf-m-plain" type="button" aria-label="Copy to clipboard" @click="copy(data)">
                                        ...
                                        <div class="pf-c-code-block__content">
                                            <pre class="pf-c-code-block__pre"><code class="pf-c-code-block__code">{{data}}</code></pre>
                                        </div>
                                    </div>
                                </div>
                            </td>
                    ...
                    <data-table-empty-state v-if="tabledata.data.data && Object.keys(tabledata.data.data).length < 1" @clear="clearFilters"></data-table-empty-state>
                </table>
