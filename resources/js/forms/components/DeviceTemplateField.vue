<template>
    <div class="pf-c-form__group">
        ...
        <div class="pf-c-input-group">
            <div class="pf-c-select" :class="errors.device_template ? 'pf-m-invalid' : ''" ref="clickOutsidetarget">
                <button
                ...
                >
                    <div class="pf-c-select__toggle-wrapper">
                        <span class="pf-c-select__toggle-text" v-if="!selected.id">Select a template</span>
                        <span class="pf-c-select__toggle-text" v-else>{{ selected.templateName }}</span>
                    </div>
                ...
                <ul class="pf-c-select__menu" role="listbox" aria-labelledby="device_template-label" v-if="showSelect ? 'hidden' : ''" style="width: auto">
                    <div class="pf-c-select__menu-group-title" aria-hidden="true" id="select-checkbox-expanded-selected-group-template">Select Template</div>
                    <div v-if="!isLoading">
                        <li role="presentation" v-for="item in models.data" :key="item.id">
                            <button class="pf-c-select__menu-item" role="option" @click.prevent="makeSelection(item.id)">
                                {{ item.templateName }}
                                <span v-if="item.id === selected.id" class="pf-c-select__menu-item-icon">
                                    <i class="fas fa-check" aria-hidden="true"></i>
                                </span>
                                <span class="pf-c-select__menu-item-description">{{ item.templateDescription }}</span>
                            </button>
                        </li>
        ...
        <p v-if="errors.device_template" class="pf-c-form__helper-text pf-m-error" id="device_template_error" aria-live="polite">
