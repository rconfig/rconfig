<template>
    <input id="editid" name="editid" type="hidden" :value="viewstate.editid" autocomplete="off" />

    <div class="pf-l-grid pf-m-gutter">
        <span class="pf-c-spinner pf-m-xl" role="progressbar" aria-label="Loading items" v-if="isLoading" style="margin: 0 auto; display: table">
        ...
        <div class="pf-l-grid__item pf-m-12-col pf-m-6-col-on-md">
            ...
            <form novalidate class="pf-c-form" v-if="!isLoading">
                ...
                <reusable-input-field
                    ...
                    :fieldname="'device_ip'"
                    :fieldtype="'text'"
                    :btnHelperTxt="'IP Address'"
                    :errors="errors"
                    :key="'device_ip'"
                ></reusable-input-field>

                <!-- VENDOR FIELD -->
                <device-vendor-field v-model="model" v-model:updateValue="model.device_vendor" :errors="errors"></device-vendor-field>

                <!-- VENDOR FIELD -->
                <device-model-field v-model="model.device_model" :errors="errors"></device-model-field>

                <!-- CATEGORY FIELD -->
                <device-category-field v-model="model" v-model:updateValue="model.device_category_id" :errors="errors"></device-category-field>

                <!-- TAG FIELD -->
                <device-tag-field v-model="model" :fieldname="'device_tag'" v-model:updateValue="model.device_tags" :errors="errors"></device-tag-field>
            </form>
        </div>
        <!-- DEVICE INFO -->
        <!-- CONNECTION INFO -->
        <div class="pf-l-grid__item pf-m-12-col pf-m-6-col-on-md">
            <form novalidate class="pf-c-form" v-if="!isLoading">
                ...
                <reusable-input-field
                    ...
                    :errors="errors"
                    :key="'device_password'"
                    ><template v-slot:btnIcon v-if="$userName === 'admin'">
                        <button tabindex="-1" class="pf-c-button pf-m-control" type="button" @click="switchVisibility(1)">
                ...
                >
                    <template v-slot:btnIcon v-if="$userName === 'admin'"
                        ><button tabindex="-1" class="pf-c-button pf-m-control" type="button" @click="switchVisibility(2)">
                            <i class="fas fa-eye" aria-hidden="true"></i>
                        </button> </template
                ></reusable-input-field>

                <!-- TEMPLATE FIELD -->
                <device-template-field v-model="model" v-model:updateValue="model.device_template" :errors="errors"></device-template-field>
                ...
                >
                    <template v-slot:tooltip-text>
                        This is the 'Privileged EXEC' prompt. You will run show commands from this prompt and you can access configure mode. Usually 'router1#'
                    </template></reusable-input-field
                >
                ...
                >
                    ...
                    <template v-slot:helper-text> <button class="pf-c-button pf-m-link pf-m-inline" type="button" @click="generatePrompts()">Auto generate prompts from device name</button></template>
                </reusable-input-field>
            </form>
        </div>
    </div>
    <!-- CONNECTION INFO -->
    <!-- ACTIONS -->
    <div class="pf-c-form__group pf-m-action" v-if="!isLoading">
