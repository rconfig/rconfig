...
- **A release is not finished when the tag is pushed. It needs a GitHub Release too.** Pushing the tag only builds and publishes the Docker images. Every tag through `core-8.2.13` shipped images with no matching Release because this step was undocumented. The `github-release` job now opens a **draft** release automatically once the images are published, so one always exists: edit it and publish it. If you are releasing by hand, `gh release create core-X.Y.Z --title "rConfig V8 Core X.Y.Z" --notes-file <file> --latest --verify-tag`.
- **CHANGELOG entries are one line each.** One bullet per change, a single sentence or two at most, saying what was wrong and what it meant for the user. No paragraphs, no root cause essays, no implementation detail: the commit message and the linked issue carry that. Credit goes at the end of the line as `Reported in #368 by <reporter>.` Entries written before this rule are long; leave them, do not reformat history.
- **Release notes are not the CHANGELOG entry.** The release body is condensed one line bullets grouped under the Keep a Changelog headings that apply, ending with `See CHANGELOG.md for detail.` Never paste the full CHANGELOG section: the release body groups and condenses, the CHANGELOG lists every change. Lead with any action the operator must take (for example, a one time command to run after upgrading).
- The full release sequence: bump the three version files, date the CHANGELOG entry, run the tests, commit, push the branch, create and push the annotated `core-` tag, then publish the draft GitHub Release once the build completes.
